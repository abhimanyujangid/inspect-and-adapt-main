export function TrainingProgressPanel({
  ...
  return (
    <section className="flex flex-col gap-4">
      <div>
        <div className="mb-2 text-[11px] font-bold uppercase tracking-wider text-primary">Progress</div>
      ...
      <div>
        <div className="mb-2 text-[11px] font-bold uppercase tracking-wider text-primary">Activity Log</div>
        <div className="min-h-[300px] rounded-sm border border-border bg-[#e8eaee] p-3 font-mono-tabular">
          <div className="flex flex-col gap-2">
            {logs.length === 0 && <p className="text-[10px] text-muted-foreground">No activity yet.</p>}
            {logs.map((l) => (
              <div
                key={l}
